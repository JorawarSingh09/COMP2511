...
int findPath (Graph g, Vertex src, Vertex dest, int max, int *path)
{
	...
	dist[src] = find_num_edges(g, src);
	//int found = false;
	pred[src] = src;

	i = 0;
	while (!QueueIsEmpty(q)){
	    //find min dist[v] and remove from queue 
	    v = find_from_queue(q, dist);
	    while (i < num){
	        if (g->edges[v][i] != 0){
                if(dist[v] + find_num_edges(g, i) < dist[i] && g->edges[v][i] <= max){
                    dist[i] = dist[v] + find_num_edges(g, i);
                    pred[i] = v;
                }   
	        }
	        i++;
...
int find_num_edges(Graph g, Vertex v){
    int i = 0;
    int count = 0;
    while(i < g->nV){
        if(g->edges[v][i]){
