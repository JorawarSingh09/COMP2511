...
TfIdfList GetSumofTf(TfIdfList newnode, TfIdfList tmp);
int check(char *v,TfIdfList new );
/*void delRepeat(TfIdfList newnode);
int checkTf(TfIdfList a, TfIdfList b);
*/
...
/*void delRepeat(TfIdfList newnode){
    TfIdfList new = newnode;
    //TfIdfList prev = NULL;
    while(new->next != NULL){
        //prev = new;
        while(checkTf(new->next, new->next->next) == 1){
            //free(new->next);
            //free(new->next->next);
            new = new->next->next->next;
        }
...
int checkTf(TfIdfList a, TfIdfList b){
    ...
    if(strcmp(a->filename, b->filename) == 0 && (a->tfIdfSum == b->tfIdfSum)){
        return 1;
    }
...
TfIdfList GetSumofTf(TfIdfList newnode, TfIdfList tmp){
    TfIdfList new = newnode;
    TfIdfList tmpnew = tmp;
    TfIdfList add = NULL;
    while(tmpnew != NULL){
        if(check(tmpnew->filename, new) == 0){
            add = newTfList(tmpnew->filename);
            add->tfIdfSum = tmpnew->tfIdfSum;

            break;
        }
        tmpnew = tmpnew->next;
    }
    TfIdfList headOFadd = add;
    tmpnew = tmp;
    while(new != NULL){
        tmpnew = tmp;
        while(tmpnew != NULL){
            if (strcmp(new->filename, tmpnew->filename) == 0 ){
                new->tfIdfSum = new->tfIdfSum + tmpnew->tfIdfSum;
            } else if (check(tmpnew->filename, newnode) == 0 && check(tmpnew->filename, headOFadd) == 0 ){

                add->next = newTfList(tmpnew->filename);
                add->next->tfIdfSum = tmpnew->tfIdfSum;
                add = add->next;
            }
            tmpnew = tmpnew->next;
        }
