...
void DLListDelete (DLList L)
{
	...
	if (L->nitems == 0){
	    ...
	    struct DLListNode *tmp = L->curr;
	    L->first = L->last = L->curr = NULL;
	    freeDLListNode(tmp);
	    L->nitems--;
	} else if (L->curr == L->first){
	    struct DLListNode *tmp = L->curr;
	    L->first = L->curr = L->first->next;
	    L->first->prev = NULL;
	    freeDLListNode(tmp);
	    L->nitems--;
	    ...
	    struct DLListNode *tmp = L->curr;
	    L->last = L->curr = L->last->prev;
	    L->last->next = NULL;
	    freeDLListNode(tmp);
	    L->nitems--;
	} else{
	    struct DLListNode *tmp = L->curr->next;
	    struct DLListNode *free = L->curr;
